...
_MART_COLUMNS = [
    ...
    "coalesce(df.steps_total, ds.steps_total) as steps_total",
    "coalesce(df.hr_min, ds.hr_min) as hr_min",
    "coalesce(df.hr_max, ds.hr_max) as hr_max",
    "coalesce(df.hrv_avg, ds.hrv_avg) as hrv_avg",
    "ds.hrv_baseline_delta as hrv_baseline_delta",
    "coalesce(df.sleep_total_minutes, ds.sleep_total_minutes) as sleep_total_minutes",
    "coalesce(df.sleep_rem_minutes, ds.sleep_rem_minutes) as sleep_rem_minutes",
    "coalesce(df.sleep_core_minutes, ds.sleep_core_minutes) as sleep_core_minutes",
    "coalesce(df.sleep_deep_minutes, ds.sleep_deep_minutes) as sleep_deep_minutes",
    "coalesce(df.sleep_awake_minutes, ds.sleep_awake_minutes) as sleep_awake_minutes",
    "coalesce(df.sleep_efficiency, ds.sleep_efficiency) as sleep_efficiency",
    "coalesce(df.spo2_avg, ds.spo2_avg) as spo2_avg",
    "ds.spo2_baseline_delta as spo2_baseline_delta",
    "coalesce(df.bp_sys_avg, ds.bp_sys_avg) as bp_sys_avg",
    "coalesce(df.bp_dia_avg, ds.bp_dia_avg) as bp_dia_avg",
    "df.kp_max as kp_max",
    "df.bz_min as bz_min",
    ...
    "coalesce(df.respiratory_rate_avg, ds.respiratory_rate_avg) as respiratory_rate_avg",
    "coalesce(df.respiratory_rate_sleep_avg, ds.respiratory_rate_sleep_avg) as respiratory_rate_sleep_avg",
    "coalesce(df.respiratory_rate_baseline_delta, ds.respiratory_rate_baseline_delta) as respiratory_rate_baseline_delta",
    "coalesce(df.temperature_deviation, ds.temperature_deviation) as temperature_deviation",
    "coalesce(df.temperature_deviation_baseline_delta, ds.temperature_deviation_baseline_delta) as temperature_deviation_baseline_delta",
    "coalesce(df.temperature_source, ds.temperature_source) as temperature_source",
    "coalesce(df.resting_hr_avg, ds.resting_hr_avg) as resting_hr_avg",
    "coalesce(df.resting_hr_baseline_delta, ds.resting_hr_baseline_delta) as resting_hr_baseline_delta",
    "coalesce(df.bedtime_consistency_score, ds.bedtime_consistency_score) as bedtime_consistency_score",
    "coalesce(df.waketime_consistency_score, ds.waketime_consistency_score) as waketime_consistency_score",
    "coalesce(df.sleep_debt_proxy, ds.sleep_debt_proxy) as sleep_debt_proxy",
    "coalesce(df.sleep_vs_14d_baseline_delta, ds.sleep_vs_14d_baseline_delta) as sleep_vs_14d_baseline_delta",
    "coalesce(df.cycle_tracking_enabled, ds.cycle_tracking_enabled) as cycle_tracking_enabled",
...
_FEATURE_DEFAULTS: Dict[str, Any] = {
    ...
    "hr_max": None,
    "hrv_avg": None,
    "hrv_baseline_delta": None,
    "spo2_avg": None,
    "spo2_baseline_delta": None,
    "bp_sys_avg": None,
    "bp_dia_avg": None,
...
async def _freshen_features(
    ...
    payload: Dict[str, Any] = {
        ...
        "hrv_baseline_delta": summary.get("hrv_baseline_delta"),
        "spo2_avg": summary.get("spo2_avg"),
        "spo2_baseline_delta": summary.get("spo2_baseline_delta"),
