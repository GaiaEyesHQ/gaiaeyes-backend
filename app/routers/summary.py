...
async def space_series(days: int = 14, request: Request, conn = Depends(get_db)):
    """
    Return timeseries for space weather (kp, bz, sw) and daily Schumann (f0/f1/f2),
    daily HR min/max, plus 5-min HR buckets for the current user.
    """
    days = max(1, min(days, 31))
    user_id = getattr(request.state, "user_id", None)

    async with conn.cursor() as cur:
        # A) space_weather: union per metric so Kp/Bz/SW appear even if on different timestamps
        await cur.execute(
            """
            (
        ...
        # B) schumann_daily: prefer tomsk > cumiana per day
        await cur.execute(
            """
            with d as (
        ...
        # D) 5-minute HR buckets (psql safe via epoch math)
        hr_ts_rows = []
        if user_id is not None:
            await cur.execute(
                """
                ...
                agg as (
                  select
                    to_timestamp(floor(extract(epoch from start_time)/300.0)*300.0)
    ...
    # Normalize to the exact JSON the app expects
    def iso(ts):
