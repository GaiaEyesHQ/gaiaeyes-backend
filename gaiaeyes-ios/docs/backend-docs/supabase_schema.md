...
-- Follow-up migration backfills landing tables and materializes `marts.schumann_daily`
-- so dependent analytics (e.g., `marts.symptom_x_space_daily`) have a guaranteed source.
...
#### Step 1 predictive ingest additions (2025-11-20)

- `ext.aurora_power` – OVATION hemispheric power samples (global, NH, SH) with source metadata to drive aurora probability/energy outlooks. Indexed on `(ts_utc, region)`.
- `ext.drap_absorption` – flattened D-RAP absorption grids (lat/lon, region, carrier) with daily rollups in `marts.drap_absorption_daily` for playbooks and cards.
- `ext.magnetometer_chain` – SuperMAG magnetometer indices and derived KP-like buckets feeding `marts.magnetometer_regional` for regional alerts.
- `ext.solar_cycle_forecast` – solar-cycle predictions (sunspot number, F10.7) backing `marts.solar_cycle_progress` for longer-term trend charts.
- `ext.coronal_hole_forecast` / `ext.sep_forecast` / `ext.cme_arrival` – landing tables for coronal-hole winds, SEP/radiation belt outlooks, and Enlil CME arrival windows; paired marts expose the consolidated outlook API.
- `marts.aurora_outlook` – daily probability/hemispheric-power outlook bands derived from OVATION feeds.
- `marts.drap_absorption_daily` – daily max/avg absorption by region/carrier distilled from the raw grids.
- `marts.magnetometer_regional` – regionalized magnetometer rollups used by `/v1/space/forecast/outlook`.
- `marts.solar_cycle_progress` – normalized solar-cycle forecast projections for trend displays.
