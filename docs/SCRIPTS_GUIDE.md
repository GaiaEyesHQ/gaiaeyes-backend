...
| `ingest_space_forecasts_step1.py` | Consolidated Step 1 ingestion covering Enlil CME runs, SEP/radiation belts, OVATION aurora power (`ext.aurora_power` + `marts.aurora_outlook`), coronal-hole forecasts, D-RAP text absorption (`ext.drap_absorption` + `marts.drap_absorption_daily`), SuperMAG magnetometer indices (`ext.magnetometer_chain` + `marts.magnetometer_regional`), and solar-cycle predictions. | `SUPABASE_DB_URL` (required unless `--dry-run`), `NASA_API`; optional `--days`, `--only`, `--user-agent`, `SUPERMAG_STATIONS` |
