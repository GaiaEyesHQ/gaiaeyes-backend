...
| W17 — Android account / app record — **EXISTING DRAFT APP CONFIRMED** | Latest September 16 screenshot: Gaia Eyes organization account, `com.gaiaeyes.app`, Draft, audience 0, updated September 7. Supersedes preceding no-app statement | Reuse the existing draft app; no duplicate enrollment/creation. [Corrected handoff](ANDROID_ACCOUNT_STATE_CORRECTION.md) separates Firebase registration/configuration, FCM delivery, signing, billing and release acceptance; local product work continues |
...
| W19 — Historical migraine import | Provider-neutral contract and local persistence foundations exist; no end-to-end provider importer or compatibility is established. **September 9 D028: Migraine Buddy Listing PDF and Bearable CSV samples received privately; project-local H09 resolved.** These personal exports are not redacted test fixtures. HealthKit history import is a different path | **Deferred behind migraine logging, follow-up, medication, editable events and calendar**, with no new ordering among that core group. Preserve reports/outreach commitments. Later review format/mapping and implement local preview/validation with synthetic or redacted derivatives; do not require a Migraine Buddy CSV or promise compatibility. See [MIGRAINE_IMPORT_REFERENCES.md](MIGRAINE_IMPORT_REFERENCES.md) |
