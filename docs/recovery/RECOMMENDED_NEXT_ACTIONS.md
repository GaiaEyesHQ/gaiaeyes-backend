...
The September 20 source/disclaimer audit in [PROJECT_STATE.md](PROJECT_STATE.md#medical-framing-and-repetitive-disclaimer-review) found no broad generic Android footer pattern, but confirmed iOS/web repetition candidates and overbroad documentation language. The historical “clinical-feeling” reason to exclude `panic` is superseded; the current pattern engine still includes it in `anxiety_day`. Assess any proposed outcome change using its scientific definition and product use. “Avoid medical claims” in the Play checklist describes present claim review, not a permanent ban on medical functionality.
