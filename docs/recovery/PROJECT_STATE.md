...
**September 10 local migraine checkpoint:** G-011/G-012 are accepted locally; G-013 saved start/end correction is [review_ready](G013_ACTIVE_WORK.md). Canonical timing, provenance/revision audit, reminder handling and corrected calendar/summary reads are implemented with explicit retry/conflict recovery in the existing iOS editor. Its 102 backend and 78 iOS checks and unsigned Release simulator boundary passed. This remains default off, with independent acceptance, physical-device/release verification and Android/member-hub time-edit parity still separate. D028 and the reports/outreach commitments remain unchanged.
