...
## September 11 saved-summary implementation checkpoint

[G-016](G016_ACTIVE_WORK.md) adds a distinct gated iOS calendar View summary action, a saved-only detail reader and complete ordered episode presentation. It preserves missing values, exact supported doses, full notes, account/generation isolation and the existing editor path. **Local verification is blocked at Xcode compiler discovery** after three preserved attempts and one bounded service recovery. The initial 99-source typecheck did not establish behavioral correctness. Independent review found G016-R01 (rejecting ordinary revision-0 episodes) and G016-R02 (mislabeling structured time as the canonical update). Both repairs are independently accepted at the source/contract level, with backend-shaped regression cases in the latest packet; executable G-016 model/UI and current Release checks remain pending. The [checkpoint packet](/Users/gennwu/Documents/Codex/2026-09-05/we/outputs/g016-saved-episode-summary/HANDOFF.md) records the source snapshot, diagnostics and resume checks. This does not complete the full update, enable the feature, authorize imports or change acquisition/product/research priorities.
