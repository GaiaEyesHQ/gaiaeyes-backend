...
**September 9 D028 — latest implementation priority:** finish migraine logging, follow-up, medication, editable events and calendar before starting imports. No new order among those five is inferred; reports and the above release-dependent outreach hold remain committed. Migraine Buddy Listing PDF and Bearable CSV samples have been received privately, closing project-local H09 availability. Compatibility and provider mapping remain unverified. See [MIGRAINE_IMPORT_REFERENCES.md](MIGRAINE_IMPORT_REFERENCES.md). This receipt does not dispatch importer, OCR, normalization or ingestion work.

## Current implementation matrix

**September 10 UTC continuation:** the coordinator accepted G-011 including G-R19 as bounded local implementation and dispatched G-012. The accepted [save-repair handoff](/Users/gennwu/Documents/Codex/2026-09-05/we/outputs/g011-gr19/HANDOFF.md) remains preserved. G-012 adds canonical paginated month/day history and navigation to the existing editor; see [verified handoff](G012_ACTIVE_WORK.md). G-012 was subsequently accepted locally by the coordinator. G-013 implements and locally verifies explicit canonical onset/end correction; see [review-ready handoff](G013_ACTIVE_WORK.md). No live feature is activated.
...
| Follow-up | **Local iOS client integrated; inactive pending review/deployment** | G-011 extends the existing iOS follow-up sheet/models/API payload for optional early signs, medicine/relief, and notes, plus later editing from symptom history. It preserves drafts on errors/conflicts, scopes drafts to the signed-in account, reuses stable retry timestamps, and accepts success only from a matching account/episode/prompt/state/content acknowledgement. A detail GET cannot prove the exact prompt answer; uncertain failures retain the draft for an explicit identical retry. The path is Debug-only and explicitly gated. | Review the local interaction, deploy the accepted G-010 migration/backend through the normal release path, then run authenticated device tests for reminder preference, snooze/dismiss, account isolation, and network failure/retry behavior before activation. Context/exposure selection remains a later UI addition; the contract already supports it. |
...
| Calendar / editable events | **G-012 accepted locally; G-013 time correction review_ready locally** | Bounded canonical episode-range read, deterministic pagination with changed-set refresh, month/day selection, explicit local timezone/as-of display, exact episode editor navigation and refresh after acknowledged editing. Existing non-migraine timeline is preserved. | G-012 is accepted locally. G-013 implements separate canonical onset/end correction with revision/token protection, original-event preservation, atomic audit/reminders and post-commit refresh. Local verification and handoff are recorded in G013_ACTIVE_WORK.md; physical-device/release acceptance and Android/member-hub parity remain. |
| Reports | **G016 saved episode summary implemented; runtime acceptance pending** | A distinct gated calendar View summary action reads one saved canonical episode, preserving ordered medicines, missingness, exact supported doses, full notes and display timezone. R01/R02 source repairs are accepted; September 13 device Debug compilation and September 14 incremental build success are now evidenced. | Complete the [focused model/UI, request/screenshot and fresh Release checks](G016_TEST_HANDOFF.md). This is not an aggregate report, PDF/CSV export or share workflow. Choose any later report/export increment explicitly; do not imply causal conclusions. |
| Import from other migraine apps | **Samples received; implementation deferred behind core tasks** | Provider-neutral contract/provenance/preview/reversal design exists. Migraine Buddy Listing PDF and Bearable CSV are preserved outside Git; they are original personal exports, not test fixtures. HealthKit history import is separate. | Finish logging, follow-up, medication, editable events and calendar first. Later review PDF/CSV mapping and build parser/preview/validation with appropriate synthetic or redacted fixtures. Provider compatibility remains unverified; no additional export request is needed now. |
...
6. **Provider adapters (deferred by D028):** samples are received for Migraine Buddy PDF and Bearable CSV. After the core group is finished, review actual mapping and create appropriate synthetic/redacted test cases before promising compatibility.
...
## September 15 G016 development-preview checkpoint

The [current preview/test handoff](G016_TEST_HANDOFF.md) reconciles the retained September 13/14 Xcode builds, 18 matching accepted source hashes, source-defined synthetic launch arguments and historical G014/G013 screenshots. It supersedes a blanket claim that the app cannot compile. G016 model/UI execution, summary screenshots/request captures and a fresh Release check remain pending. H13 is complete; H14 applies only to its separate prepared Terminal diagnostic. No native run or new owner action is requested in this reconciliation. Android structured parity, authenticated/device acceptance, release activation and imports remain separate.

## September 9 G-011 review-repair checkpoint

G-R15 through G-R18 are repaired and locally verified, pending coordinator acceptance. Untouched medicine/sign records and metadata are preserved; backend Decimal strings and timestamp normalization round-trip correctly. Existing detail GETs cannot prove the exact prompt answer, so only the matching successful acknowledgement or identical explicit retry confirms a save. The actual sheet/history editor passed six UI tests; the focused client/workflow tests covered 30 cases. Release remains disabled and the synthetic fixture is excluded from the Release binary. See [G011_ACTIVE_WORK.md](G011_ACTIVE_WORK.md) for the current handoff.
...
- **D028 supersedes the missing-export blocker:** Migraine Buddy PDF and Bearable CSV samples are received privately. Mapping, parsing, preview, validation and provider compatibility remain unverified, and all import implementation is deferred behind the specified core group. Raw personal files must not become repository fixtures or external-service inputs.
